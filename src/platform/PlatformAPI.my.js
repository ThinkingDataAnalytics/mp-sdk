...
var currentPlatform = my;
import {AutoTrackBridge} from './AutoTrack.mp';
...
class PlatformAPI {
    ...
    static request(options) {
        return currentPlatform.request({
            url: options.url,
            data: options.data,
    ...
    static getAppOptions(callback) {
        if (_.isFunction(callback) && _.isFunction(currentPlatform.onAppShow)) {
            try {
                currentPlatform.onAppShow(callback);
            } catch (e) {
